...
@section('content')
    <div class="mb-8">
        <h1 class="text-2xl font-bold text-slate-900">إضافة تخصص — {{ $department->name }}</h1>
    ...
    <div class="max-w-xl rounded-3xl border border-white/50 bg-white/45 p-6 shadow-xl backdrop-blur-xl">
        <form method="POST" action="{{ route('admin.departments.specializations.store', $department) }}" class="space-y-4">
            @csrf
            <div>
