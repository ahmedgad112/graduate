...
@section('content')
    <div class="mb-8 flex flex-wrap items-center justify-between gap-4">
        <div>
            ...
            <p class="mt-1 text-sm text-slate-600">أضف الأقسام ثم أدر التخصصات لكل قسم من الرابط «التخصصات».</p>
        </div>
        <a href="{{ route('admin.departments.create') }}" class="rounded-xl bg-emerald-600 px-4 py-2 text-sm font-semibold text-white shadow-md hover:bg-emerald-700">إضافة قسم</a>
    </div>
            ...
            <table class="min-w-full divide-y divide-white/50 text-sm">
                <thead class="bg-white/50 text-right text-xs font-semibold uppercase tracking-wide text-slate-500">
                    <tr>
                        ...
                        <th class="px-4 py-3">عدد التخصصات</th>
                ...
                <tbody class="divide-y divide-white/40 bg-white/30">
                    @foreach ($departments as $department)
                        <tr class="hover:bg-white/40">
                            ...
                            <td class="px-4 py-3 text-slate-700">{{ $department->specializations_count }}</td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex flex-wrap justify-center gap-2">
                                    <a href="{{ route('admin.departments.specializations.index', $department) }}" class="rounded-lg bg-sky-600/90 px-3 py-1 text-xs font-semibold text-white hover:bg-sky-700">التخصصات</a>
                                    <a href="{{ route('admin.departments.edit', $department) }}" class="rounded-lg border border-white/60 bg-white/60 px-3 py-1 text-xs font-semibold text-slate-800 hover:bg-white/80">تعديل</a>
                                    <form method="POST" action="{{ route('admin.departments.destroy', $department) }}" onsubmit="return confirm('حذف هذا القسم؟');">
                                        @csrf
                                        @method('DELETE')
